...
class FourDRADDet(Detector3DTemplate):
    """
    Detector that augments PointPillar with a radar Cluster Enhancement Branch and
    Cross-Attention Feature Fusion as described in 4DRadDet.
    """
    ...
    def build_ceb(self, model_info_dict):
        """
        Build the radar Cluster Enhancement Branch if specified in the config.
        """
    ...
    def build_fusion_module(self, model_info_dict):
        """
        Build the fusion module that combines PEB and CEB BEV features via CAFF.
        """
        ...
        if hasattr(fusion_cfg, 'CHANNELS'):
