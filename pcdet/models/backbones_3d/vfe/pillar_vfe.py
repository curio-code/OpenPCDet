...
class PillarEnhancementBranch(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)
        ...
        ## check if config has the correct params, if not, throw exception
        radar_config_params = ["USE_RCS", "USE_VR", "USE_VR_COMP", "USE_TIME", "USE_ELEVATION", "USE_VELOCITY_DIFF", "VELOCITY_SOURCE"]
        ...
        if all(hasattr(self.model_cfg, attr) for attr in radar_config_params):
            ...
            self.use_time = self.model_cfg.USE_TIME
            self.use_elevation = self.model_cfg.USE_ELEVATION
            self.use_velocity_diff = self.model_cfg.USE_VELOCITY_DIFF
            self.velocity_diff_source = self.model_cfg.VELOCITY_SOURCE
        ...
        num_point_features += 6  # center_x, center_y, center_z, mean_x, mean_y, mean_z, time, we need 6 new
        if self.use_velocity_diff:
            num_point_features += 1                # v_c
        ...
        # Configure velocity-difference feature v_c if requested
        # Decide which velocity source to use
        self.vel_src_ind = None
        if self.use_velocity_diff:
            # If config specifies a source, respect it; otherwise prefer compensated velocity if enabled
            source = self.velocity_diff_source
            if source is None:
                source = 'v_r_comp' if self.use_vr_comp else ('v_r' if self.use_vr else None)
            if source == 'v_r_comp' and self.use_vr_comp:
                self.vel_src_ind = self.vr_comp_ind
            elif source == 'v_r' and self.use_vr:
                self.vel_src_ind = self.vr_ind
            else:
                raise Exception("USE_VELOCITY_DIFF is True but a valid VELOCITY_SOURCE ('v_r' or 'v_r_comp') is not available/enabled")
        ...
        # account for optional distance feature dimension in PFN input
        if self.with_distance:
            num_point_features += 1
        ...
        for k in self.selected_indexes:
            print(str(k) + ": " + self.available_features[k])
        if self.use_velocity_diff:
            print("Including velocity-difference feature v_c from: " + self.available_features[self.vel_src_ind])
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        # calculate center for each pillar and save points' offset from the center. voxel_coordinate * voxel size + offset should be the center of pillar (coords are indexed backwards)
        f_center = torch.zeros_like(orig_xyz)
        f_center[:, :, 0] = raw_features[:, :, self.x_ind] - (
                    coords[:, 3].to(voxel_features.dtype).unsqueeze(1) * self.voxel_x + self.x_offset)
        f_center[:, :, 1] = raw_features[:, :, self.y_ind] - (
                    coords[:, 2].to(voxel_features.dtype).unsqueeze(1) * self.voxel_y + self.y_offset)
        f_center[:, :, 2] = raw_features[:, :, self.z_ind] - (
                    coords[:, 1].to(voxel_features.dtype).unsqueeze(1) * self.voxel_z + self.z_offset)

        # Ensure index tensor is on same device for advanced indexing
        if self.selected_indexes.device != voxel_features.device:
            selected_idx = self.selected_indexes.to(voxel_features.device)
        else:
            selected_idx = self.selected_indexes
        voxel_features = voxel_features[:, :, selected_idx]  # filtering for used features
        
        features = [voxel_features, f_cluster]

        if self.use_velocity_diff:
            # Compute per-pillar mean over real points (padding is zero so sum/num works)
            vel_vals = raw_features[:, :, self.vel_src_ind:self.vel_src_ind + 1]
            vel_mean = vel_vals.sum(dim=1, keepdim=True) / voxel_num_points.type_as(vel_vals).view(-1, 1, 1)
            v_c = vel_vals - vel_mean  # (num_voxels, T, 1)
            features.append(v_c)
